...
public class ManagerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action.equals("4"))//派单
		{
			ConnDB con=new ConnDB();
			int cow=con.Scount("select count(*) from Odd where state='0'");
			con.close();
			ConnDB co=new ConnDB();
			int row=co.Scount("select count(*) from Staff where State='1'");
			co.close();
			if(row!=0&&cow!=0)
			{
				ConnDB conne=new ConnDB();
				conne.paid(row,cow);
				conne.close();
			}
			DoPage dopage=new DoPage();
