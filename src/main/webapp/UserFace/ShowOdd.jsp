...
<div class="container" style=" text-align: center; margin-top:20px;">
  ...
  <div style="width:50%; margin-bottom:10px">
   <form action="/mypaidan/UserServlet?page=1&action=2" method="post">  
      ...
      <tr>
	   ...
	   <td height="26px">
	     <c:if test="${odd.state == '0' }">
	         <p class="bg-success text-white">未委派</p>
	     </c:if>
	   ...
	   <td height="26px">
	     <c:if test="${odd.state == '0' }">
	         <a href="/mypaidan/UserServlet?action=3&oddnumber=${odd.oddNumber }" class="btn btn-primary btn-sm" role="button">撤&nbsp;销</a>
	     </c:if>
	     <c:if test="${odd.state == '1' }">
	         <a href="/mypaidan/UserServlet?action=4&oddnumber=${odd.oddNumber }" class="btn btn-primary btn-sm" role="button">完&nbsp;成</a>
	     </c:if>
	     <c:if test="${odd.state == '2' }">
	         <a href="/mypaidan/UserFace/Evaluate.jsp?oddnumber=${odd.oddNumber }" class="btn btn-primary btn-sm" role="button">评&nbsp;价</a>
	     </c:if>
	     <c:if test="${odd.state == '3' }">
	         <a href="/mypaidan/UserServlet?action=3&oddnumber=${odd.oddNumber }" class="btn btn-primary btn-sm" role="button">撤&nbsp;销</a>
	     </c:if>
 ...
 <div class="container" style=" text-align: center; margin:0px auto;">
      ...
      <br>
       <ul class="pagination float-md-right">
          <li class="page-item"> 
             <a class="page-link" href="/mypaidan/UserServlet?action=2&page=1">首页</a>
          </li>
          <li class="page-item">
             <%--如果当前页面不是第一页，显示上一页选项 --%>
             <c:if test="${dopage.nowPage>1 }">
               <a class="page-link" href="/mypaidan/UserServlet?action=2&page=${dopage.nowPage-1 }">上一页</a>
             </c:if>
          </li>
          <li class="page-item"> 
             <%--如果当前页面不是最后一页，显示下一页选项 --%>
             <c:if test="${dopage.nowPage<dopage.totalPage}">
                <a class="page-link" href="/mypaidan/UserServlet?action=2&page=${dopage.nowPage+1 }">下一页</a>
             </c:if>
          </li>
          <li class="page-item">
            <%--显示末页 --%>
            <a class="page-link" href="/mypaidan/UserServlet?action=2&page=${dopage.totalPage }">末页</a>
          </li>
          <li class="page-item">
